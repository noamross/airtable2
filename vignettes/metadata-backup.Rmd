...
When creating new tables or bases programmatically, you need to supply Airtable
field specification lists. Rather than building them by hand, you can infer them
from an existing R data frame.

`air_infer_fields()` maps R column types to Airtable field types:

| R type | Airtable type |
|---|---|
| `character` | `singleLineText` |
| `integer` | `number` (precision 0) |
| `double` | `number` (precision 8) |
| `logical` | `checkbox` |
| `Date` | `date` (ISO format) |
| `factor` | `singleSelect` (choices from `levels()`) |
...
df <- data.frame(
  ...
  Completed = as.Date("2026-01-15")
)

fields <- air_infer_fields(df)
# [[1]] $name = "Name",      $type = "singleLineText"
# [[2]] $name = "Score",     $type = "number", $options = list(precision = 0)
# [[3]] $name = "Passed",    $type = "checkbox"
# [[4]] $name = "Category",  $type = "singleSelect", $options = list(choices = ...)
# [[5]] $name = "Completed", $type = "date", $options = list(dateFormat = list(name = "iso"))
```

`air_infer_table()` wraps this into a complete table spec:


``` r
tbl_spec <- air_infer_table(df, "Exam Results", description = "Mid-year exam data")
at_create_table(tbl_spec$name, fields = tbl_spec$fields, base_id = "appXXXXXX")
```

For the common case of writing a data frame to a table that may not exist yet,
`air_write()` wraps this automatically with `create_table = TRUE`:


``` r
air_write(df, "Exam Results", "appXXXXXX", create_table = TRUE)
```

You can also pass multiple table specs to `at_create_base()` to provision an
entire new base from a list of data frames:


``` r
tables <- list(
  air_infer_table(artists_df, "Artists"),
  air_infer_table(projects_df, "Projects")
)
at_create_base("My New Base", tables = tables, workspace_id = "wspXXXXXX")
```
...
`air_meta_sync()` takes documentation one step further. The recommended
workflow:

1. Call `air_meta_init()` to create a `_metadata` table in your base,
   populated with the current schema.
2. Open the base in Airtable and edit the description text directly in the
   `_metadata` table — no R required.
3. Call `air_meta_sync()` to read the updated `_metadata` table and push the
   changes back to the field descriptions.
...
`air_meta_sync()` is idempotent — running it again updates existing
descriptions and adds new rows for any fields added since the last sync.
...
(`restore_linked_fields = TRUE`), after all records are created it runs a
two-step pass:

1. **Field definitions** — `multipleRecordLinks` fields are recreated with
   their `linkedTableId` remapped to the new base's table IDs.
2. **Cell values** — the link values from the dump (old record IDs) are
   remapped to the new record IDs using the insertion-order mapping, then
...
**When automatic re-linking might not be sufficient:** the automatic approach
matches records by insertion order, which works when the dump and restore are
done as a unit. If you are merging records from multiple sources, or if some
tables were partially restored, you may need to re-link manually using a
natural key:
